...
document.addEventListener("DOMContentLoaded", function () {
  // =========================
  // Dropdown helpers (shared)
  // =========================
  function ddOpen(el, open) {
    if (!el) return;
    el.classList.toggle("show", open);
    el.classList.toggle("open", open); // supports both CSS conventions
  }
  ...
  // Return true if click target is inside ANY of the provided elements
  function isInsideAny(target, els) {
  ...
  function setProfileChevron(open) {
  ...
  function closeProfile() {
    ...
    ddOpen(profileDropdown, false);
    setProfileChevron(false);
  }

  function closeNotifications() {
    if (!notificationDropdown) return;
    ddOpen(notificationDropdown, false);
  }

  function toggleProfile(e) {
    if (!profileDropdown || (!profileBtn && !profileImg)) return;
    const next = !ddIsOpen(profileDropdown);
    ddOpen(profileDropdown, next);
    setProfileChevron(next);

    // If profile opens, close notification to avoid overlap
    if (next && notificationDropdown && ddIsOpen(notificationDropdown)) {
      closeNotifications();
    }
  ...
  function toggleNotifications(e) {
    ...
    const next = !ddIsOpen(notificationDropdown);
    ddOpen(notificationDropdown, next);

    // If notifications open, close profile to avoid overlap
    if (next && profileDropdown && ddIsOpen(profileDropdown)) {
      closeProfile();
    }
  ...
  // (Capture phase so it works even if inner elements stopPropagation)
  // =========================
  window.addEventListener(
    ...
    (e) => {
      ...
      // Close profile when click outside profile elements
      if (
        profileDropdown &&
        ddIsOpen(profileDropdown) &&
        !isInsideAny(target, [profileDropdown, profileBtn, profileImg])
      ) {
        closeProfile();
      }

      // Close notifications when click outside notification elements
      if (
        notificationDropdown &&
        ddIsOpen(notificationDropdown) &&
        !isInsideAny(target, [notificationDropdown, notificationBtn])
      ) {
        closeNotifications();
      }
    },
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (profileDropdown && ddIsOpen(profileDropdown)) {
      closeProfile();
      closedAny = true;
    }
    if (notificationDropdown && ddIsOpen(notificationDropdown)) {
      closeNotifications();
      closedAny = true;
    }
  ...
  // Close profile on link click (existing behavior, safer)
  if (profileDropdown) {
    profileDropdown.querySelectorAll("a").forEach((link) => {
      link.addEventListener("click", () => closeProfile());
    });
  }

  // Optimistic badge decrement when user clicks a notif item
  // (server will mark read on edit page)
  if (notificationDropdown) {
    notificationDropdown.addEventListener("click", (e) => {
      const item = e.target.closest(".notif-item");
      if (!item) return;

      // Visually fade item and decrement badge once
      item.style.opacity = "0.7";
      if (notificationBadge) {
        const n = parseInt((notificationBadge.textContent || "").trim(), 10);
        if (!Number.isNaN(n) && n > 0) {
          notificationBadge.textContent = String(n - 1);
        }
      }
  ...
  // =========================================================
  // === Sidebar: edge orb toggle (position + magnetic + persist)
  // (kept from your existing file)
  // =========================================================
